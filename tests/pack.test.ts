...
const state = (): InternalState => ({
  ...
  threads: [], replies: [], projects: [], inquiries: [], notifications: [], audit: [], mutations: [], media: [],
});

describe('portable packs', () => {
  it('creates a public pack without credentials and restores stable IDs', () => {
    const source = state();
    const pack = createPublicPack(source);
    ...
    restorePublicPack(pack, target);
    expect(target.posts[0].publicId).toBe('post-1');
    expect(target.credentials).toHaveLength(0);
  });

  it('encrypts operator credentials and restores them without sessions', () => {
    const source = state();
    const pack = createOperatorPack(source, 'a sufficiently long operator passphrase');
    ...
    const target = state();
    restoreOperatorPack(pack, 'a sufficiently long operator passphrase', target);
