...
import { currentUser, publicState, createCredential, contentSlug, hasMutation, hasRole, hashToken, id, isValidPassword, newSession, normalizeEmail, readState, recordAudit, rememberMutation, verifyPassword, writeState } from '@/lib/store';
import type { ActionName, InternalState, Inquiry, Post, Project, Thread, User } from '@/lib/types';
...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === 'posts.save') {
      ...
      post.title = title;
      post.slug = slug;
      post.excerpt = stringValue(payload, 'excerpt');
      post.bodyMarkdown = stringValue(payload, 'bodyMarkdown');
      post.status = payload.status === 'published' ? 'published' : 'draft';
      post.featuredAssetId = optionalString(payload, 'featuredAssetId');
      post.tagIds = Array.isArray(payload.tagIds) ? payload.tagIds.filter((tag): tag is string => typeof tag === 'string') : [];
      post.revision += 1;
      ...
      if (!user) throw new Error('Sign in required');
      const timestamp = Date.now();
      const thread: Thread = { publicId: id(), categoryId: stringValue(payload, 'categoryId') || 'general', authorId: user.publicId, title: stringValue(payload, 'title'), status: 'open', pinned: false, locked: false, postCount: 1, lastPostAt: timestamp, lastPostPublicId: null, createdAt: timestamp, updatedAt: timestamp };
      state.threads.push(thread);
      const reply = { publicId: id(), threadId: thread.publicId, parentPostId: null, authorId: user.publicId, bodyMarkdown: stringValue(payload, 'bodyMarkdown'), revision: 1, editedAt: null, createdAt: timestamp, deletedAt: null, moderationState: 'visible' as const };
      ...
      const timestamp = Date.now();
      const reply = { publicId: id(), threadId: thread.publicId, parentPostId: optionalString(payload, 'parentPostId'), authorId: user.publicId, bodyMarkdown: stringValue(payload, 'bodyMarkdown'), revision: 1, editedAt: null, createdAt: timestamp, deletedAt: null, moderationState: 'visible' as const };
      state.replies.push(reply);
      thread.postCount += 1;
      ...
      project.title = title;
      project.slug = slug;
      project.summary = stringValue(payload, 'summary');
